...
def readWord(address):
    ...
    for i in range(ASSOCIATIVITY):
        if cache.sets[index].blocks[i].tag == tag:
            print("read hit: adress = ", address, " index: ", index, " block index: ", i, " tag: ", tag, " offset: ",
                  offset)
    ...
    for m in range(ASSOCIATIVITY):
        if cache.sets[index].blocks[m].valid == False:
            ...
            print("read miss: adress = ", address, " index: ", index, " block index: ", i, " tag: ", tag, " offset: ",
    ...
    for n in range(ASSOCIATIVITY):  # update for part 2
        ...
        print("read miss + replace: adress = ", address, " index: ", index, " block index: ", i, " tag: ", tag,
              " offset: ", offset, " new range: ", rangeStart, "-", rangeStart + CACHE_BLOCK_SIZE - 1)
        return memory[address] + 256 * memory[address + 1] + 256 ** 2 * memory[address + 2] + 256 ** 3 * memory[
            address + 3]
